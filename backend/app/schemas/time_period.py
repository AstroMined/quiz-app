...
class TimePeriodSchema(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=20, description="Name of the time period"
    )
    ...
    def check_id_name_combination(self):
        ...
        if self.name != id_name_map.get(self.id):
            raise ValueError(
                f"Invalid combination of id and name. For id {self.id}, name should be {id_name_map.get(self.id)}"
            )
        return self
