...
class RoleBaseSchema(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=200, description="Description of the role"
    )
    default: bool = Field(
...
class RoleCreateSchema(RoleBaseSchema):
    permissions: List[str] = Field(
        ...,
        min_items=1,
        description="List of permission names associated with the role",
    )
...
class RoleUpdateSchema(BaseModel):
    name: Optional[str] = Field(
        None, min_length=1, max_length=50, description="Name of the role"
    )
    description: Optional[str] = Field(
        None, max_length=200, description="Description of the role"
    )
    ...
    default: Optional[bool] = Field(
        None, description="Whether this is the default role for new users"
    )
...
class RoleSchema(RoleBaseSchema):
    ...
    def convert_permissions(cls, v):
        if isinstance(v, list) and all(isinstance(item, str) for item in v):
            ...
            raise ValueError(
                "Permissions must be a list of strings or objects with 'name' attribute"
            )
