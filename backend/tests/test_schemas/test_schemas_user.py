...
def test_user_create_schema_password_validation():
    ...
    with pytest.raises(ValidationError) as exc_info:
        UserCreateSchema(
            username="testuser", password="nodigits!", email="test@example.com"
        )
    assert "Password must contain at least one digit" in str(exc_info.value)

    with pytest.raises(ValidationError) as exc_info:
        UserCreateSchema(
            username="testuser", password="nouppercase123!", email="test@example.com"
        )
    assert "Password must contain at least one uppercase letter" in str(exc_info.value)

    with pytest.raises(ValidationError) as exc_info:
        UserCreateSchema(
            username="testuser", password="NOLOWERCASE123!", email="test@example.com"
        )
    assert "Password must contain at least one lowercase letter" in str(exc_info.value)

    with pytest.raises(ValidationError) as exc_info:
        UserCreateSchema(
            username="testuser", password="NoSpecialChar123", email="test@example.com"
        )
...
def test_user_create_schema_username_validation():
    ...
    with pytest.raises(ValidationError) as exc_info:
        UserCreateSchema(
            username="invalid@user", password="ValidPass123!", email="test@example.com"
        )
...
def test_user_create_schema_email_validation():
    with pytest.raises(ValidationError) as exc_info:
        UserCreateSchema(
            username="testuser", password="ValidPass123!", email="invalidemail"
        )
