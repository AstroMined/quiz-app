...
async def test_get_current_user_expired_token(db_session, test_model_user):
    ...
    expired_token = jwt.encode(to_encode, settings_core.SECRET_KEY, algorithm=settings_core.ALGORITHM)
...
async def test_get_current_user_nonexistent_user(db_session):
    ...
    token = jwt.encode(to_encode, settings_core.SECRET_KEY, algorithm=settings_core.ALGORITHM)
