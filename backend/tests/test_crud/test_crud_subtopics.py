...
def test_create_topic_to_subtopic_association(
    db_session, test_schema_subtopic, test_schema_topic
):
    subtopic = create_subtopic_in_db(db_session, test_schema_subtopic.model_dump())
...
def test_delete_topic_to_subtopic_association(
    db_session, test_schema_subtopic, test_schema_topic
):
    subtopic = create_subtopic_in_db(db_session, test_schema_subtopic.model_dump())
    topic = create_topic_in_db(db_session, test_schema_topic.model_dump())
    create_topic_to_subtopic_association_in_db(db_session, topic.id, subtopic.id)
    assert (
        delete_topic_to_subtopic_association_from_db(db_session, topic.id, subtopic.id)
        is True
    )


def test_create_subtopic_to_concept_association(
    db_session, test_schema_subtopic, test_schema_concept
):
    subtopic = create_subtopic_in_db(db_session, test_schema_subtopic.model_dump())
...
def test_create_question_to_subtopic_association(
    db_session, test_schema_subtopic, test_schema_question
):
    subtopic = create_subtopic_in_db(db_session, test_schema_subtopic.model_dump())
    question = create_question_in_db(db_session, test_schema_question.model_dump())
    assert (
        create_question_to_subtopic_association_in_db(
            db_session, question.id, subtopic.id
        )
        is True
    )


def test_delete_question_to_subtopic_association(
    db_session, test_schema_subtopic, test_schema_question
):
    subtopic = create_subtopic_in_db(db_session, test_schema_subtopic.model_dump())
    question = create_question_in_db(db_session, test_schema_question.model_dump())
...
def test_read_topics_for_subtopic(db_session, test_schema_subtopic, test_schema_topic):
    ...
    create_topic_to_subtopic_association_in_db(
        db_session, created_topic.id, subtopic.id
    )
...
def test_read_concepts_for_subtopic(
    db_session, test_schema_subtopic, test_schema_concept
):
    subtopic = create_subtopic_in_db(db_session, test_schema_subtopic.model_dump())
    concept = create_concept_in_db(db_session, test_schema_concept.model_dump())
...
def test_read_questions_for_subtopic(
    db_session, test_schema_subtopic, test_schema_question
):
    subtopic = create_subtopic_in_db(db_session, test_schema_subtopic.model_dump())
    question = create_question_in_db(db_session, test_schema_question.model_dump())
