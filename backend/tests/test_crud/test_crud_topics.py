...
def test_create_subject_to_topic_association(
    db_session, test_schema_topic, test_schema_subject
):
    topic = create_topic_in_db(db_session, test_schema_topic.model_dump())
...
def test_create_topic_to_subtopic_association(
    db_session, test_schema_topic, test_schema_subtopic
):
    topic = create_topic_in_db(db_session, test_schema_topic.model_dump())
...
def test_create_question_to_topic_association(
    db_session, test_schema_topic, test_schema_question
):
    topic = create_topic_in_db(db_session, test_schema_topic.model_dump())
...
def test_delete_question_to_topic_association(
    db_session, test_schema_topic, test_schema_question
):
    topic = create_topic_in_db(db_session, test_schema_topic.model_dump())
    question = create_question_in_db(db_session, test_schema_question.model_dump())
    create_question_to_topic_association_in_db(db_session, question.id, topic.id)
    assert (
        delete_question_to_topic_association_from_db(db_session, question.id, topic.id)
        is True
    )
