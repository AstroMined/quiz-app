...
def test_update_discipline(db_session, test_schema_discipline):
    ...
    updated_discipline = update_discipline_in_db(
        db_session, discipline.id, updated_data
    )
...
def test_create_domain_to_discipline_association(
    db_session, test_schema_discipline, test_schema_domain
):
    discipline = create_discipline_in_db(
        db_session, test_schema_discipline.model_dump()
    )
...
def test_create_discipline_to_subject_association(
    db_session, test_schema_discipline, test_schema_subject
):
    discipline = create_discipline_in_db(
        db_session, test_schema_discipline.model_dump()
    )
...
def test_read_domains_for_discipline(
    db_session, test_schema_discipline, test_schema_domain
):
    discipline = create_discipline_in_db(
        db_session, test_schema_discipline.model_dump()
    ...
    create_domain_to_discipline_association_in_db(
        db_session, created_domain.id, discipline.id
    )
...
def test_read_subjects_for_discipline(
    db_session, test_schema_discipline, test_schema_subject
):
    discipline = create_discipline_in_db(
        db_session, test_schema_discipline.model_dump()
