...
def test_get_users_pagination(logged_in_client, db_session, test_model_role):
    ...
    for i in range(15):
        ...
        create_user_in_db(db_session, {**user_data.model_dump(), "hashed_password": hashed_password})
...
def test_update_user_me_no_changes(logged_in_client, test_model_user_with_group):
    ...
    assert response.status_code == 200
    updated_user = response.json()
    assert updated_user["username"] == test_model_user_with_group.username.lower()  # Username stored as lowercase
    assert updated_user["email"] == test_model_user_with_group.email

def test_create_user_duplicate_username(logged_in_client, test_model_user_with_group, test_model_role):
    user_data = {
        "username": test_model_user_with_group.username,
...
def test_create_user_duplicate_email(logged_in_client, test_model_user_with_group, test_model_role):
    user_data = {
...
def test_update_user_me_partial(logged_in_client, test_model_user_with_group):
    update_data = {
        "username": "partial_update_username"
    }
    response = logged_in_client.put("/users/me", json=update_data)
    assert response.status_code == 200
    updated_user = response.json()
    assert updated_user["username"] == "partial_update_username"
    assert updated_user["email"] == test_model_user_with_group.email  # Email should remain unchanged
...
def test_update_user_me_same_email(logged_in_client, test_model_user_with_group):
    update_data = {
        "email": test_model_user_with_group.email  # Same as current email
    }
    response = logged_in_client.put("/users/me", json=update_data)
    assert response.status_code == 200
    updated_user = response.json()
...
def test_create_user_password_hashing(logged_in_client, test_model_role, db_session):
    ...
    db_user = db_session.query(UserModel).filter(UserModel.id == created_user["id"]).first()
