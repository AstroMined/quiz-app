...
def test_create_leaderboard_entry(logged_in_client, test_model_user_with_group, time_period_daily):
    group_id = test_model_user_with_group.groups[0].id
    entry_data = {
        "user_id": test_model_user_with_group.id,
        "score": 100,
...
def test_update_leaderboard_entry(logged_in_client, test_model_user_with_group, time_period_daily):
    ...
    update_response = logged_in_client.put(f"/leaderboard/{created_entry['id']}", json=update_data)
...
def test_delete_leaderboard_entry(logged_in_client, test_model_user_with_group, time_period_daily):
    # First, create a leaderboard entry
    group_id = test_model_user_with_group.groups[0].id
    entry_data = {
        "user_id": test_model_user_with_group.id,
    ...
    get_response = logged_in_client.get(f"/leaderboard/user/{test_model_user_with_group.id}")
...
def test_get_leaderboard_with_limit(logged_in_client, setup_test_data, time_period_daily):
    response = logged_in_client.get(f"/leaderboard/?time_period={time_period_daily.id}&limit=5")
    logger.debug(response.json())
    assert response.status_code == 200
...
def test_update_leaderboard_entry_invalid_data(logged_in_client, test_model_user_with_group, time_period_daily):
    ...
    update_response = logged_in_client.put(f"/leaderboard/{created_entry['id']}", json=invalid_update_data)
