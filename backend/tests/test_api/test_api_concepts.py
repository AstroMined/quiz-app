...
def test_update_concept(logged_in_client, test_model_concept, test_model_subtopic):
    ...
    response = logged_in_client.put(f"/concepts/{test_model_concept.id}", json=update_data)
    assert response.status_code == 200
    updated_concept = response.json()
    assert updated_concept["name"] == "Updated Concept"
    assert test_model_subtopic.id in [subtopic["id"] for subtopic in updated_concept["subtopics"]]
