...
def test_create_user_response_invalid_question(
    logged_in_client, test_model_user_with_group, test_model_questions
):
    invalid_data = {
...
def test_create_user_response_invalid_answer(
    logged_in_client, test_model_user_with_group, test_model_questions
):
    invalid_data = {
        "user_id": test_model_user_with_group.id,
...
def test_create_and_score_user_response(
    logged_in_client, test_model_user, test_model_questions
):
    logger.debug(
        "test_questions 1: %s", sqlalchemy_obj_to_dict(test_model_questions[0])
    )
    correct_answer = next(
        ac for ac in test_model_questions[0].answer_choices if ac.is_correct
    )
    response_data = {
    ...
    ), f"Failed to create user response. Status: {create_response.status_code}, Response: {create_response.json()}"

    created_response = create_response.json()
    assert (
        "id" in created_response
    ), f"Response does not contain 'id'. Response: {created_response}"
    assert (
        created_response["is_correct"] is True
    ), f"Response should be scored as correct. Response: {created_response}"


def test_create_and_score_incorrect_user_response(
    logged_in_client, test_model_user, test_model_questions
):
    incorrect_answer = next(
        ac for ac in test_model_questions[0].answer_choices if not ac.is_correct
    )
    response_data = {
        "user_id": test_model_user.id,
        "question_id": test_model_questions[0].id,
    ...
    assert (
        created_response["is_correct"] is False
    ), f"Response should be scored as incorrect. Response: {created_response}"
...
def test_update_user_response(logged_in_client, test_model_user, test_model_questions):
    ...
    update_data = {"response_time": 30}  # Update response time
...
def test_delete_user_response(
    ...
    created_response = logged_in_client.post(
        "/user-responses/", json=response_data
    ).json()
...
def test_create_user_response_missing_data(
    logged_in_client, test_model_user, test_model_questions
):
    invalid_data = {
        "user_id": test_model_user.id,
...
def test_get_user_responses_with_pagination(
    ...
    correct_answer_1 = next(
        ac for ac in test_model_questions[0].answer_choices if ac.is_correct
    )
    correct_answer_2 = next(
        ac for ac in test_model_questions[1].answer_choices if ac.is_correct
    )
    response_data_1 = {
        "user_id": test_model_user.id,
        "question_id": test_model_questions[0].id,
...
def test_create_and_retrieve_user_response(
    logged_in_client, test_model_user, test_model_questions
):
    correct_answer = next(
        ac for ac in test_model_questions[0].answer_choices if ac.is_correct
    ...
    retrieve_response = logged_in_client.get(
...
def test_update_nonexistent_user_response(
    logged_in_client, test_model_user_with_group, test_model_questions
):
    update_data = {"response_time": 30}
