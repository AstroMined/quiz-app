...
def test_register_user_success(client, test_model_role):
    ...
    assert response.status_code == 201, f"User registration failed. Response: {response.json()}"
...
def test_register_user_invalid_password(client):
    ...
    assert "Value should have at least 8 items after validation" in response.json()["detail"][0]["msg"]

def test_register_user_missing_digit_in_password(client):
    """Test registration with a password missing a digit."""
    user_data = {"username": "newuser", "password": "NoDigitPassword!", "email": "newuser@example.com"}
    response = client.post("/register", json=user_data)
    assert response.status_code == 422
...
def test_register_user_missing_uppercase_in_password(client):
    ...
    user_data = {"username": "newuser", "password": "nouppercasepassword123!", "email": "newuser@example.com"}
    response = client.post("/register", json=user_data)
    assert response.status_code == 422
...
def test_register_user_missing_lowercase_in_password(client):
    ...
    user_data = {"username": "newuser", "password": "NOLOWERCASEPASSWORD123!", "email": "newuser@example.com"}
    response = client.post("/register", json=user_data)
    assert response.status_code == 422
...
def test_register_user_password_hashing(client, test_model_role, db_session):
    ...
    db_user = db_session.query(UserModel).filter(UserModel.username == "hashed_password_user").first()
...
def test_register_user_special_characters_in_username(client, test_model_role):
    ...
    response = client.post("/register", json=user_data)
    assert response.status_code == 422
    assert "Username must contain only alphanumeric characters, hyphens, underscores, and periods" in response.json()["detail"][0]["msg"]
