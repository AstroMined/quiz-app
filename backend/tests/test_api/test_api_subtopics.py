...
def test_create_subtopic(logged_in_client, test_model_topic):
    subtopic_data = {"name": "Test Subtopic", "topic_ids": [test_model_topic.id]}
...
def test_update_subtopic(logged_in_client, test_model_subtopic, test_model_topic):
    update_data = {"name": "Updated Subtopic", "topic_ids": [test_model_topic.id]}

    response = logged_in_client.put(
        f"/subtopics/{test_model_subtopic.id}", json=update_data
    )
    assert response.status_code == 200
...
def test_update_nonexistent_subtopic(logged_in_client, test_model_topic):
    update_data = {"name": "Nonexistent Subtopic", "topic_ids": [test_model_topic.id]}
