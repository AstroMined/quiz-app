...
def test_setup_filter_questions_data(db_session, setup_filter_questions_data):
    ...
    assert (
        db_session.query(DomainModel).filter(DomainModel.name == "Science").first()
        is not None
    )
    assert (
        db_session.query(DomainModel).filter(DomainModel.name == "Mathematics").first()
        is not None
    )

    assert (
        db_session.query(DisciplineModel)
        .filter(DisciplineModel.name == "Physics")
        .first()
        is not None
    )
    assert (
        db_session.query(DisciplineModel)
        .filter(DisciplineModel.name == "Pure Mathematics")
        .first()
        is not None
    )

    assert (
        db_session.query(SubjectModel)
        .filter(SubjectModel.name == "Classical Mechanics")
        .first()
        is not None
    )
    assert (
        db_session.query(SubjectModel).filter(SubjectModel.name == "Algebra").first()
        is not None
    )

    assert (
        db_session.query(TopicModel).filter(TopicModel.name == "Newton's Laws").first()
        is not None
    )
    assert (
        db_session.query(TopicModel).filter(TopicModel.name == "Linear Algebra").first()
        is not None
    )

    assert (
        db_session.query(SubtopicModel)
        .filter(SubtopicModel.name == "First Law of Motion")
        .first()
        is not None
    )
    assert (
        db_session.query(SubtopicModel)
        .filter(SubtopicModel.name == "Second Law of Motion")
        .first()
        is not None
    )
    assert (
        db_session.query(SubtopicModel).filter(SubtopicModel.name == "Matrices").first()
        is not None
    )
    assert (
        db_session.query(SubtopicModel)
        .filter(SubtopicModel.name == "Vector Spaces")
        .first()
        is not None
    )

    assert (
        db_session.query(ConceptModel).filter(ConceptModel.name == "Inertia").first()
        is not None
    )
    assert (
        db_session.query(ConceptModel)
        .filter(ConceptModel.name == "Force and Acceleration")
        .first()
        is not None
    )
    assert (
        db_session.query(ConceptModel)
        .filter(ConceptModel.name == "Matrix Operations")
        .first()
        is not None
    )
    assert (
        db_session.query(ConceptModel)
        .filter(ConceptModel.name == "Linear Independence")
        .first()
        is not None
    )

    assert (
        db_session.query(QuestionTagModel)
        .filter(QuestionTagModel.tag == "physics")
        .first()
        is not None
    )
    assert (
        db_session.query(QuestionTagModel)
        .filter(QuestionTagModel.tag == "mathematics")
        .first()
        is not None
    )
    assert (
        db_session.query(QuestionTagModel)
        .filter(QuestionTagModel.tag == "mechanics")
        .first()
        is not None
    )
    assert (
        db_session.query(QuestionTagModel)
        .filter(QuestionTagModel.tag == "linear algebra")
        .first()
        is not None
    )

    assert (
        db_session.query(QuestionSetModel)
        .filter(QuestionSetModel.name == "Physics Question Set")
        .first()
        is not None
    )
    assert (
        db_session.query(QuestionSetModel)
        .filter(QuestionSetModel.name == "Math Question Set")
        .first()
        is not None
    )
    ...
    newton_first_law_question = (
        db_session.query(QuestionModel)
        .filter(QuestionModel.text == "What is Newton's First Law of Motion?")
        .first()
    )
    ...
    assert set(
        [question_tag.tag for question_tag in newton_first_law_question.question_tags]
    ) == {"physics", "mechanics"}

    linear_independence_question = (
        db_session.query(QuestionModel)
        .filter(
            QuestionModel.text
    ...
    assert set(
        [
            question_tag.tag
            for question_tag in linear_independence_question.question_tags
        ]
...
def test_filter_questions(logged_in_client, db_session):
    ...
    if questions:
        subject = (
            db_session.query(SubjectModel).filter(SubjectModel.name == "Math").first()
        )
        topic = (
            db_session.query(TopicModel).filter(TopicModel.name == "Algebra").first()
        )
        subtopic = (
            db_session.query(SubtopicModel)
            .filter(SubtopicModel.name == "Linear Equations")
            .first()
        )
        for question in questions:
...
def test_filter_questions_by_subject(logged_in_client, db_session):
    response = logged_in_client.get("/questions/filter", params={"subject": "Math"})
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
    assert isinstance(questions, list)
...
def test_filter_questions_by_topic(logged_in_client, db_session):
    response = logged_in_client.get("/questions/filter", params={"topic": "Algebra"})
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
    assert isinstance(questions, list)
...
def test_filter_questions_by_subtopic(logged_in_client, db_session):
    response = logged_in_client.get(
        "/questions/filter", params={"subtopic": "Linear Equations"}
    )
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
    assert isinstance(questions, list)
...
def test_filter_questions_by_difficulty(logged_in_client, db_session):
    response = logged_in_client.get("/questions/filter", params={"difficulty": "Easy"})
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
    assert isinstance(questions, list)
...
def test_filter_questions_by_single_tag(logged_in_client, db_session):
    response = logged_in_client.get(
        "/questions/filter", params={"question_tags": ["equations"]}
    )
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
    assert isinstance(questions, list)
    question_tag = (
        db_session.query(QuestionTagModel)
        .filter(QuestionTagModel.tag == "equations")
        .first()
    )
    assert all(
        question_tag.id in [t.id for t in question["question_tags"]]
        for question in questions
    )
...
def test_filter_questions_by_tags(logged_in_client, db_session):
    response = logged_in_client.get(
        "/questions/filter", params={"question_tags": ["geometry"]}
    )
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
    assert isinstance(questions, list)
    question_tag = (
        db_session.query(QuestionTagModel)
        .filter(QuestionTagModel.tag == "geometry")
        .first()
    )
    assert all(
        question_tag.id in [t.id for t in question["question_tags"]]
...
def test_filter_questions_by_multiple_criteria(logged_in_client, db_session):
    response = logged_in_client.get(
        "/questions/filter",
        params={"subject": "Math", "topic": "Algebra", "difficulty": "Easy"},
    )
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
    assert isinstance(questions, list)
...
def test_filter_questions_with_pagination(
    logged_in_client, db_session, setup_filter_questions_data
):
    response = logged_in_client.get(
        "/questions/filter", params={"subject": "Math", "skip": 1, "limit": 2}
    )
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
...
def test_filter_questions_no_results(
    logged_in_client, db_session, setup_filter_questions_data
):
    response = logged_in_client.get(
        "/questions/filter", params={"subject": "NonexistentSubject"}
    )
    assert response.status_code == 200, f"Failed with response: {response.json()}"
    questions = response.json()
...
def test_filter_questions_endpoint_invalid_params(logged_in_client, db_session):
    response = logged_in_client.get(
        "/questions/filter", params={"invalid_param": "value"}
    )
