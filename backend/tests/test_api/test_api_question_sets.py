...
def test_create_private_question_set(logged_in_client):
    data = {"name": "Test Private Set", "is_public": False}
    response = logged_in_client.post("/question-sets/", json=data)
    print(response.json())
    assert response.status_code == 201
...
def test_read_question_sets(logged_in_client, test_model_question_set):
    ...
    assert any(
        qs["id"] == test_model_question_set.id
        and qs["name"] == test_model_question_set.name
        for qs in response.json()
    )
...
def test_update_question_set_not_found(logged_in_client):
    ...
    response = logged_in_client.put(
        f"/question-sets/{question_set_id}", json=question_set_update
    )
    ...
    json_data = [
        {
            ...
            "subtopic_ids": [
                subtopic.id for subtopic in test_model_questions[0].subtopics
            ],
            "concept_ids": [concept.id for concept in test_model_questions[0].concepts],
            "difficulty": test_model_questions[
                0
            ].difficulty.value,  # Use the string value of the enum
            "answer_choices": [
                {
                    "text": choice.text,
...
def test_upload_question_set_invalid_json(logged_in_client):
    ...
    with tempfile.NamedTemporaryFile(
        ...
        response = logged_in_client.post(
            ...
            files={
                "file": ("invalid.json", open(temp_file.name, "rb"), "application/json")
            },
        )
...
def test_create_question_set_with_existing_name(
    logged_in_client, test_model_question_set
):
...
def test_retrieve_question_set_with_questions(
    logged_in_client, test_model_question_set
):
...
def test_update_question_set_endpoint(
    logged_in_client, test_model_question_set, test_model_questions
):
    data = {
    ...
    response = logged_in_client.put(
        f"/question-sets/{test_model_question_set.id}", json=data
    )
    logger.debug("response: %s", response.json())
    assert response.status_code == 200
    assert response.json()["name"] == "Updated Question Set"
    assert any(
        question["id"] == test_model_questions[0].id
...
def test_delete_question_set(logged_in_client, test_model_question_set, db_session):
    ...
    if isinstance(question_sets_after_deletion, list):
        assert not any(
            qs["id"] == question_set_id for qs in question_sets_after_deletion
        ), "Question set was not deleted."
    elif (
        isinstance(question_sets_after_deletion, dict)
        and "detail" in question_sets_after_deletion
    ):
        assert (
            question_sets_after_deletion["detail"] == "No question sets found."
        ), "Unexpected response after deletion."
    else:
        raise AssertionError(
            "Unexpected response format after attempting to delete the question set."
        )
...
def test_update_question_set_with_multiple_questions(
    logged_in_client, test_model_question_set, test_model_questions
):
    ...
    data = {
        "name": "Updated Question Set",
        "question_ids": [test_question_1.id, test_question_2.id],
    }
    response = logged_in_client.put(
        f"/question-sets/{test_model_question_set.id}", json=data
    )
    ...
    assert response.json()["name"] == "Updated Question Set"
    assert any(
        question["id"] == test_question_1.id
        for question in response.json()["questions"]
    )
    assert any(
        question["id"] == test_question_2.id
        for question in response.json()["questions"]
    )


def test_update_question_set_remove_questions(
    logged_in_client, test_model_question_set
):
...
def test_update_question_set_invalid_question_ids(
    logged_in_client, test_model_question_set
):
    data = {
        "name": "Updated Question Set",
        "question_ids": [999],
    }  # Assuming question with ID 999 doesn't exist
    response = logged_in_client.put(
        f"/question-sets/{test_model_question_set.id}", json=data
    )
    assert response.status_code == 400
