...
def test_create_group_long_description(logged_in_client, test_model_user, db_session):
    ...
    response = logged_in_client.post("/groups", json=group_data)
    assert response.status_code == 422
    assert "String should have at most 500 characters" in response.json()["detail"][0]["msg"]
...
def test_get_nonexistent_group(logged_in_client):
    response = logged_in_client.get("/groups/99999")  # Assuming 99999 is a non-existent group ID
    assert response.status_code == 404
    assert response.json()["detail"] == "Group not found"

def test_update_nonexistent_group(logged_in_client):
    update_data = {"name": "Updated Nonexistent Group"}
    response = logged_in_client.put("/groups/99999", json=update_data)  # Assuming 99999 is a non-existent group ID
    assert response.status_code == 404
    assert response.json()["detail"] == "Group not found"

def test_delete_nonexistent_group(logged_in_client):
    response = logged_in_client.delete("/groups/99999")  # Assuming 99999 is a non-existent group ID
    assert response.status_code == 404
    assert response.json()["detail"] == "Group not found"

def test_update_group_not_creator(logged_in_client, test_model_group, db_session, test_model_role):
    ...
    new_user_db = create_user_in_db(db_session, {**new_user.model_dump(), "hashed_password": hashed_password})
    ...
    headers = {"Authorization": f"Bearer {access_token}"}
    response = logged_in_client.put(f"/groups/{test_model_group.id}", json=update_data, headers=headers)
    assert response.status_code == 403
    assert response.json()["detail"] == "Only the group creator can update the group"

def test_delete_group_not_creator(logged_in_client, test_model_group, db_session, test_model_role):
    # Create a new user who is not the creator of the group
    from backend.app.crud.crud_user import create_user_in_db
    from backend.app.schemas.user import UserCreateSchema
    ...
    headers = {"Authorization": f"Bearer {access_token}"}
    response = logged_in_client.delete(f"/groups/{test_model_group.id}", headers=headers)
    assert response.status_code == 403
...
def test_update_group_partial(logged_in_client, test_model_group):
    ...
    assert updated_group["description"] == test_model_group.description  # Description should remain unchanged
