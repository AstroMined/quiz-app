...
def test_get_answer_choice(logged_in_client, test_model_answer_choices):
    response = logged_in_client.get(f"/answer-choices/{test_model_answer_choices[0].id}")
    assert response.status_code == 200
    ...
    assert retrieved_answer_choice["is_correct"] == test_model_answer_choices[0].is_correct
...
def test_update_answer_choice(logged_in_client, test_model_answer_choices):
    ...
    response = logged_in_client.put(f"/answer-choices/{test_model_answer_choices[0].id}", json=update_data)
    assert response.status_code == 200
...
def test_delete_answer_choice(logged_in_client, test_model_answer_choices):
    response = logged_in_client.delete(f"/answer-choices/{test_model_answer_choices[0].id}")
    assert response.status_code == 204

    # Verify that the answer choice has been deleted
    get_response = logged_in_client.get(f"/answer-choices/{test_model_answer_choices[0].id}")
