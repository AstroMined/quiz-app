...
def test_settings_core():
    ...
    assert all(
        isinstance(endpoint, str) for endpoint in settings_core.UNPROTECTED_ENDPOINTS
    )
...
def test_load_settings_validation_error(
    mock_settings_core, mock_load_config, mock_dotenv_values
):
    ...
    mock_settings_core.side_effect = CustomValidationError(
        [{"loc": ("field",), "msg": "error message", "type": "value_error"}]
    )
...
def test_load_settings_unexpected_error(mock_load_config, mock_dotenv_values):
    ...
    with patch(
        "backend.app.core.config.SettingsCore",
