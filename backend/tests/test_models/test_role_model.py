...
def test_role_permission_relationship(db_session):
    try:
        ...
        retrieved_role = (
            db_session.query(RoleModel).filter(RoleModel.name == "Test Role").first()
        )

        assert retrieved_role is not None, "Role not found in database"
        assert (
            len(retrieved_role.permissions) == 2
        ), f"Expected 2 permissions, found {len(retrieved_role.permissions)}"
        assert (
            permission1 in retrieved_role.permissions
        ), "Permission 1 not found in role's permissions"
        assert (
            permission2 in retrieved_role.permissions
        ), "Permission 2 not found in role's permissions"
