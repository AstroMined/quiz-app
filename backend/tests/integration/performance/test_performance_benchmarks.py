...
def test_api_endpoint_performance_benchmark(client, test_model_user, performance_tracker):
...
def test_performance_baseline_validation(performance_tracker):
...
def test_performance_regression_detection(performance_tracker):
    ...
    try:
        ...
        # In parallel execution, workers may have different data, so just verify report generation works
        assert len(report) > 0
...
def test_concurrent_performance_benchmark(db_session, test_model_user, test_model_role, performance_tracker):
...
def test_memory_efficiency_validation(db_session, test_model_user, performance_tracker):
