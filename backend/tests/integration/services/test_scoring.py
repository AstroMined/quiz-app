...
def test_calculate_leaderboard_scores(db_session):
    ...
    responses = [
        UserResponseModel(
            user_id=user1.id, is_correct=True, timestamp=now - timedelta(hours=1)
        ),
        UserResponseModel(
            user_id=user1.id, is_correct=True, timestamp=now - timedelta(days=2)
        ),
        UserResponseModel(
            user_id=user2.id, is_correct=True, timestamp=now - timedelta(hours=2)
        ),
    ]
    db_session.add_all(responses)
    db_session.commit()
