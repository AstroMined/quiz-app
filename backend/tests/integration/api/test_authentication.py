...
def test_login_case_insensitive_username(
    client, test_model_user, db_session, test_model_default_role
):
    # Test with uppercase username
    login_data = {
        "username": test_model_user.username.upper(),
    ...
    login_data = {
        "username": test_model_user.username.lower(),
        "password": "TestPassword123!",
    }
    response = client.post("/login", json=login_data)
    assert response.status_code == 200
    ...
    mixed_case_username = "".join(
        [
            c.upper() if i % 2 == 0 else c.lower()
            for i, c in enumerate(test_model_user.username)
        ]
    )
...
def test_logout_all_sessions(logged_in_client, client, test_model_user, db_session):
    ...
    for token in tokens:
        ...
        ), f"Expected 401 Unauthorized when using a revoked token, got {response.status_code}"
    ...
    time.sleep(
        1.1
    )  # Sleep for 1.1 seconds to ensure we're past the 1-second grace period
    ...
    ), "Expected successful login after logout all"
