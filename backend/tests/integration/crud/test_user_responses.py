...
def test_create_user_response(
    db_session,
    test_schema_user_response,
    test_user_data,
    test_schema_question,
    test_schema_answer_choice,
):
...
def test_read_user_responses(
    ...
    responses = read_user_responses_from_db(db_session, filters=filters)
...
def test_update_user_response(
    ...
    updated_response = update_user_response_in_db(
        db_session, user_response.id, updated_data
    )
    ...
    assert (
        updated_response.is_correct != original_is_correct
    ), f"Updated response is_correct ({updated_response.is_correct}) should not equal original is_correct ({original_is_correct})"
    assert (
        fetched_response.is_correct != original_is_correct
    ), f"Fetched response is_correct ({fetched_response.is_correct}) should not equal original is_correct ({original_is_correct})"
...
def test_read_user_responses_for_user(
    db_session,
    test_schema_user_response,
    test_user_data,
    test_schema_question,
...
def test_read_user_responses_for_question(
    db_session,
    test_schema_user_response,
    test_user_data,
    test_schema_question,
    ...
    create_user_response_in_db(db_session, user_response_data)
    question_responses = read_user_responses_for_question_from_db(
...
def test_read_user_responses_with_time_range(
    db_session,
    test_schema_user_response,
    test_user_data,
    test_schema_question,
    ...
    past_filters = {"start_time": past_start_time, "end_time": past_end_time}

    past_responses = read_user_responses_from_db(db_session, filters=past_filters)
